...
it("renders renders bouth buttons correctly", async () => {
  render(<PaginationButtons setPage={setPage} page={2} maxPage={3} />);

  expect(screen.getAllByRole("button")).toHaveLength(2);
});

it("dose not render prev button when is in first page", () => {
  render(<PaginationButtons setPage={setPage} page={1} maxPage={3} />);
...
it("dose not render next button when is in last page", () => {
  render(<PaginationButtons setPage={setPage} page={3} maxPage={3} />);
