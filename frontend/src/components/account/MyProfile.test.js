...
describe("Mi frofile component", () => {
  ...
  it("display user info correctly", () => {
    expect(
      screen.getByText(currentUser.name, { exact: false }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(currentUser.email, { exact: false }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(currentUser.number, { exact: false }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(currentUser.address, { exact: false }),
    ).toBeInTheDocument();
  });
